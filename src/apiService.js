...
const getFromRepo = async (path, queryParams = {}) => {
    return await getRateLimit().then(rateLimit => {
        ...
        if(rateLimit.rate.remaining === 0) throw new Error(`Superaste la cantidad de request que podes hacer.\nIntenta de nuevo ${moment(rateLimit.rate.reset * 1000).fromNow().toString()}`)
...
export const getReadme = async (dirName) => {
    if(!dirName) throw new Error("Se necesita el nombre de la carpete")
...
export const getMediaLink = (dirName, mediaName) => {
    if(!mediaName.startsWith(".")) return mediaName
    let validExtension = ['jpg', 'png', 'gif', 'pdf'];
    if(!dirName) throw new Error("Se necesita el nombre de la carpeta")
    if(!mediaName) throw new Error("Se necesita un nombre para el archivo")
    if(!validExtension.includes(mediaName.split(".").slice(-1)[0])) throw new Error("Asegurate de pasar el nombre del archivo incluyendo la extension")
...
export const getUrlLink = (dirName, link) => {
    ...
    if(!dirName) throw new Error("Se necesita el nombre de la carpeta")
    if(!link) throw new Error("Se necesita un nombre para el archivo")
